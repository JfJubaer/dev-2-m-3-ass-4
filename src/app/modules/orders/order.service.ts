...
const createorder = async (payload:Iorder): Promise<Iorder | null> => {
    ...
    try {
      ...
      //array
      const newCow  = await Cow.findOneAndUpdate({_id:payload.cow},{'label':'sold out'}, { session });
     ...
     const newbuyer  = await User.findOneAndUpdate({_id:payload?.buyer},{'budget':buyermoney}, { session });
  
     if (!newbuyer) {
       throw new ApiError(httpStatus.BAD_REQUEST, 'Failed to update buyer');
     }
     const newSeller  = await User.findOneAndUpdate({_id:cow?.seller},{'income':sellermoney}, { session });
